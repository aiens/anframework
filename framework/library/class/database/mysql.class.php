...
final class mysql {
	...
	public function test(){
		echo 'Mysql driver has been loaded.';
	}
	...
	public function get_one($data, $table, $where = '', $order = '', $group = '') {
		//echo 'now is mysql\'s get_noe work.';
		$where = $where == '' ? '' : ' WHERE '.$where;
		$order = $order == '' ? '' : ' ORDER BY '.$order;
		$group = $group == '' ? '' : ' GROUP BY '.$group;
		$limit = ' LIMIT 1';
	 ...
	 * 						为字符串时[例：`name`='anf',`hits`=`hits`+1]。
	 *						为数组时[例: array('name'=>'anf','password'=>'123456')]
	...
	public function halt($message = '', $sql = '') {
		...
		if($this->config['debug']){
			...
			$e['message'] = 'Error:'.$this->errno();
		}
		require_once ANF_PATH.'library/output/database_error.php';
		exit;		
	}
	...
	public function select_sql($data,$sql,$order,$limit = '') {
		$limit = $limit == '' ? '' : ' LIMIT '.$limit;
		$order = $order == '' ? '' : ' ORDER BY '.$order;
		$sql = 'select '.$data.' from '.$sql.$order.$limit;
		$this->execute($sql);
		if(!is_resource($this->lastqueryid)) {
			return $this->lastqueryid;
		}
